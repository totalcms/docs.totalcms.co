...
- [Vite](/builder/frontend/) (run `tcms builder:frontend` to scaffold it)
- Tailwind CSS
...
Both install the same Vite scaffold to `<projectRoot>/frontend/`. See [`builder:frontend`](/builder/cli#builderfrontend/) for the full reference.
...
4. **Reorder pages** — use the admin's drag-drop reorder mode to set the navigation order (see [Reordering Pages](/builder/admin#reordering-pages/))
...
The starter's pages and any seed content live in `jumpstart.json`, in the standard [JumpStart](/cli/jumpstart/) format. List the page records first, in the order you want them to appear in the navigation:
